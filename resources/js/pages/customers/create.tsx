...
import { Search, Plus, MoreVertical, Filter, ArrowUpRight, Mail, AlertTriangle } from "lucide-react";
        ...
        <div className="p-2 sm:p-4 space-y-2">
          {customers.data.length === 0 ? (
            ...
            customers.data.map((customer) => (
              <div
                key={customer.id}
                className={`group flex flex-col sm:flex-row sm:items-center justify-between p-4 bg-background border border-transparent hover:border-border hover:shadow-sm rounded-xl transition-all duration-200 gap-4 ${!customer.is_active ? 'opacity-75 grayscale-[0.5]' : ''}`}
              >
                {/* Left: Avatar & Identity */}
                <div className="flex items-center gap-4 min-w-0 flex-1">
                  <Avatar className="h-10 w-10 sm:h-12 sm:w-12 border bg-indigo-50 dark:bg-indigo-950/50">
                  ...
                  <div className="flex flex-col truncate">
                    <div className="flex items-center gap-2">
                      <Link href={`/customers/${customer.id}`} className="font-semibold text-sm sm:text-base text-foreground hover:text-primary transition-colors truncate">
                        {customer.name}
                      </Link>
                      {customer.is_active ? (
                    ...
                    <DropdownMenuContent align="end" className="w-48">
                      ...
                      {customer.is_active ? (
                         ...
                         <DropdownMenuItem 
                           ...
                           className="text-emerald-600 focus:text-emerald-600 focus:bg-emerald-50 dark:focus:bg-emerald-950/50"
                         >
                           Reactivate Account
                         </DropdownMenuItem>
                      )}
        ...
        <DialogContent className="sm:max-w-[425px]">
          <form onSubmit={handleCreateCustomer}>
            <DialogHeader>
              ...
              <DialogDescription>
                Create a new customer profile. An empty wallet will be generated automatically.
              </DialogDescription>
            </DialogHeader>
